...
class Tweet(base.Item):
    ...
    tcooutlinksss = base._DeprecatedProperty(
        "tcooutlinksss", lambda self: " ".join(self.tcooutlinks), "tcooutlinks"
    )
            ...
            if (contentP := tweet.find("p", "tweet-text")) :
                ...
                for a in contentP.find_all("a"):
                    if (
                        ...
                        and (not a.has_attr("class") or "u-hidden" not in a["class"])
                    ):
...
class TwitterAPIScraper(base.Scraper):
        ...
        while True:
            ...
            for instruction in obj["timeline"]["instructions"]:
                ...
                for entry in entries:
                    if entry["entryId"] == "sq-cursor-bottom" or entry[
                        "entryId"
                    ].startswith("cursor-bottom-"):
                        newCursor = entry["content"]["operation"]["cursor"]["value"]
                        if (
                            "stopOnEmptyResponse"
                            in entry["content"]["operation"]["cursor"]
                        ):
                            stopOnEmptyResponse = entry["content"]["operation"][
                                "cursor"
                            ]["stopOnEmptyResponse"]
            if (
                not newCursor
                or newCursor == cursor
                or (stopOnEmptyResponse and self._count_tweets(obj) == 0)
            ):
                # End of pagination
                break
            cursor = newCursor
            ...
            for entry in entries:
                if entry["entryId"].startswith("sq-I-t-") or entry[
                    ...
                    if "tweet" in entry["content"]["item"]["content"]:
                        if (
                            "promotedMetadata"
                            in entry["content"]["item"]["content"]["tweet"]
                        ...
                        if (
                            ...
                            logger.warning(
                                f'Skipping tweet {entry["content"]["item"]["content"]["tweet"]["id"]} which is not in globalObjects'
                            )
                            continue
                        ...
                        "tombstone" in entry["content"]["item"]["content"]
                        and "tweet" in entry["content"]["item"]["content"]["tombstone"]
                    ):
                        if (
                            ...
                            logger.warning(
                                f'Skipping tweet {entry["content"]["item"]["content"]["tombstone"]["tweet"]["id"]} which is not in globalObjects'
                            )
                            continue
                        tweet = obj["globalObjects"]["tweets"][
                            entry["content"]["item"]["content"]["tombstone"]["tweet"][
    ...
    def _tweet_to_tweet(self, tweet, obj):
        ...
        kwargs["user"] = self._user_to_user(
            obj["globalObjects"]["users"][tweet["user_id_str"]]
        )
        kwargs["date"] = email.utils.parsedate_to_datetime(tweet["created_at"])
        kwargs["outlinks"] = (
            [u["expanded_url"] for u in tweet["entities"]["urls"]]
            if "urls" in tweet["entities"]
            else []
        )
        kwargs["tcooutlinks"] = (
            [u["url"] for u in tweet["entities"]["urls"]]
            if "urls" in tweet["entities"]
            else []
        )
        kwargs[
            "url"
        ] = f'https://twitter.com/{obj["globalObjects"]["users"][tweet["user_id_str"]]["screen_name"]}/status/{kwargs["id"]}'
            ...
            for medium in tweet["extended_entities"]["media"]:
                if medium["type"] == "photo":
                    ...
                    if format not in ("jpg", "png"):
                        logger.warning(
                            f'Skipping photo with unknown format on tweet {kwargs["id"]}: {format!r}'
                        )
                        continue
                    media.append(
                        Photo(
                    ...
                    for variant in medium["video_info"]["variants"]:
                        variants.append(
                            VideoVariant(
                                ...
                                url=variant["url"],
                                bitrate=variant.get("bitrate") or None,
                            )
                        )
                    mKwargs = {
                    ...
                    if medium["type"] == "video":
                        mKwargs["duration"] = (
                            medium["video_info"]["duration_millis"] / 1000
                        )
                        cls = Video
        ...
        kwargs["mentionedUsers"] = (
            ...
            and tweet["entities"]["user_mentions"]
            else None
        )
        return Tweet(**kwargs)
    ...
    def _user_to_user(self, user):
        ...
        kwargs["descriptionUrls"] = [
            ...
            for x in user["entities"]["description"].get("urls", [])
        ]
        ...
        kwargs["linkUrl"] = (
            (user["entities"]["url"]["urls"][0].get("expanded_url") or user.get("url"))
...
class TwitterSearchScraper(TwitterAPIScraper):
    ...
    def get_items(self):
        ...
        for obj in self._iter_api_data(
            "https://api.twitter.com/2/search/adaptive.json", params, paginationParams
        ):
...
class TwitterUserScraper(TwitterSearchScraper):
    ...
    def _get_entity(self):
        ...
        params = {
            "variables": json.dumps(
                {"screen_name": self._username, "withHighlightedLabel": True},
                separators=(",", ":"),
            )
        }
        ...
        description = self._render_text_with_urls(
        ...
        return User(
            ...
            linkUrl=user["legacy"]["entities"]["url"]["urls"][0]["expanded_url"]
    ...
    def setup_parser(cls, subparser):
        ...
        subparser.add_argument(
            "username", type=username, help="A Twitter username (without @)"
        )
